...
class ChunkedTokenDatabase:
    ...
    def prepare_block_info(
        self, start: int, end: int, block_ids: list[int], kv_cache_group_id: int = 0
    ) -> tuple[int, list[int]]:
        block_size = self.get_block_size(kv_cache_group_id)
        block_id = block_ids[start // block_size] if block_ids else 0
        block_len = self.group_block_len.get(kv_cache_group_id, [])
        size_list = []
    ...
    def prepare_value_layer(
        self, start: int, end: int, block_ids: list[int], layer_id: int, kv_cache_group_id: int = 0
    ):
        group_block_size = self.get_block_size(kv_cache_group_id)
        block_id = block_ids[start // group_block_size] if block_ids else 0
        addr_list: list[int] = []
        size_list: list[int] = []
        ...
        if length == 0:
            return addr_list, size_list, block_id
        num_layers_in_group = self.group_num_layers.get("kv", {}).get(kv_cache_group_id, 0)
        if num_layers_in_group <= 0:
            caches_per_layer = length
        else:
            caches_per_layer = max(1, length // num_layers_in_group)
        base_offset = layer_id * caches_per_layer
        for i in range(caches_per_layer):
            idx = base_offset + i
            if idx >= len(group_addrs):
                break
...
class ReqMeta:
    ...
    def __init__(
        ...
        if block_ids_by_group_np is None and block_ids_by_group:
            self.block_ids_by_group_np = [np.asarray(ids, dtype=np.int64) for ids in block_ids_by_group]
        else:
...
class SharedBlockData:
    """Pre-compute block data shared across all layers for the same request."""
