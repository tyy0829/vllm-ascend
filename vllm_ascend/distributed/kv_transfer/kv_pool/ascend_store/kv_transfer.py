...
class LayerBatchBuilder:
    ...
    def _require_request_arrays(
        ...
        if is_save:
            if (
                request.block_ids_by_group_np is not None
                and request.block_gvas_by_group_np is not None
            ...
            if (
                request.block_ids_by_group_np is not None
                and request.load_block_gvas_by_group_np is not None
                and group_id < len(request.load_block_gvas_by_group_np)
            ):
                block_ids_np = request.block_ids_by_group_np[group_id]
...
class KVCacheStoreLayerSendingThread(KVTransferThread):
    def __init__(
        ...
        self.group_builders: list[LayerBatchBuilder] | None = None
