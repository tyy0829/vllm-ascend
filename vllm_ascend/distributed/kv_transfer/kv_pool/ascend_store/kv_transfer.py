...
class LayerBatchBuilder:
    def __init__(
        ...
        num_layers: int,
        group_id: int = 0,
        group_num_layers: int | None = None,
        group_block_size: int = 0,
        ...
        group_block_len = token_database.group_block_len.get(group_id, token_database.group_block_len.get(0, []))
        self._block_len_np = np.asarray(group_block_len, dtype=np.int64)
        group_base_addr = token_database.group_kv_caches_base_addr.get(
            group_id, token_database.group_kv_caches_base_addr.get(0, [])
        )
        self._kv_caches_base_addr_np = np.asarray(group_base_addr, dtype=np.int64)
        group_block_stride = token_database.group_block_stride.get(
            group_id, token_database.group_block_stride.get(0, group_block_len)
        )
        self._block_stride_np = np.asarray(group_block_stride, dtype=np.int64)
        if group_num_layers is not None:
            self._group_num_layers = group_num_layers
        else:
            self._group_num_layers = num_layers
        self._caches_per_layer = max(
            1, self._block_len_np.shape[0] // max(1, self._group_num_layers)
        )
        self._block_ids_buf: np.ndarray | None = None
        self._block_gvas_buf: np.ndarray | None = None
        self.slot_offset: int = 0
    ...
    def _build_transfer_arrays(
        ...
        logger.debug(
            "_build_transfer_arrays: group=%d layer_id=%d page_size=%d num_blocks=%d "
            "rank_layer_offset=%d caches_per_layer=%d "
            "block_ids[:3]=%s gvas[:3]=%s addrs[:3]=%s sizes[:3]=%s",
            self.group_id, layer_id, self.page_size_bytes, len(block_ids_arr),
            rank_layer_offset, caches_per_layer,
            block_ids_arr[:3].tolist(), base_gvas_arr[:3].tolist(),
            addr_arr.ravel()[:3].tolist(), size_arr.ravel()[:3].tolist(),
        )
    ...
    def _require_request_arrays(
        ...
        if (request.block_gvas_by_group_np is not None
            ...
            if request.block_gvas_np is None:
                raise RuntimeError("ReqMeta numpy block GVA metadata is not initialized")
    ...
    def build_shared(self, task: LayerTransferTask) -> SharedBlockData | None:
        ...
        for block_range in task.block_ranges:
            block_ids_np, _ = self._require_request_arrays(block_range, self.group_id)
            actual_end = min(block_range.end_block, len(block_ids_np))
            total += actual_end - block_range.start_block
        ...
        for block_range in task.block_ranges:
            ...
            block_ids_np, block_gvas_np = self._require_request_arrays(block_range, self.group_id)

            logger.debug(
                "build_shared: group=%d req=%s block_ids_len=%d block_gvas_len=%d "
                "start_block=%d end_block=%d partial=%s",
                self.group_id, request.req_id, len(block_ids_np), len(block_gvas_np),
                block_range.start_block, block_range.end_block, block_range.partial_block_index,
            )
            num_blocks = block_range.end_block - block_range.start_block
            if num_blocks > 0:
                gva_start = block_range.start_block
                gva_end = min(block_range.end_block, len(block_ids_np), len(block_gvas_np))
                actual_blocks = gva_end - gva_start
                if actual_blocks > 0:
                    end = offset + actual_blocks
                    block_ids_arr[offset:end] = block_ids_np[gva_start:gva_end]
                    block_gvas_arr[offset:end] = block_gvas_np[gva_start:gva_end]
                    offset = end

            if block_range.partial_block_index is not None:
                assert request.last_block_gva is not None
                partial_idx = block_range.partial_block_index
                if partial_idx >= len(block_ids_np):
                    partial_idx = len(block_ids_np) - 1 if len(block_ids_np) > 0 else -1
                if partial_idx >= 0:
                    block_ids_arr[offset] = block_ids_np[partial_idx]
    ...
    def build_addrs(
        ...
        layer_idx_in_group = layer_id - self.slot_offset
        addr_array, size_array, gvas_array = self._build_transfer_arrays(
            shared.block_ids_arr, shared.block_gvas_arr, layer_idx_in_group
        )
...
class KVCacheStoreLayerSendingThread(KVTransferThread):
    def __init__(
        ...
        if group_builders is not None:
            ...
            self.layer_batch_builder = group_builders[0] if group_builders else LayerBatchBuilder(
                token_database, my_key_index, num_ranks_per_layer, page_size_bytes, num_layers,
            )
        else:
            ...
            self.group_builders = [self.layer_batch_builder]
    ...
    def build_shared_data(self, task: LayerTransferTask) -> SharedBlockData | None:
        ...
        if group_id < len(self.group_builders):
            return self.group_builders[group_id].build_shared(task)
    ...
    def _handle_request(  # type: ignore[override]
        ...
        if shared is None:
            ...
            assert not self.layer_save_finished_events[layer_id].is_set(), f"thread: {layer_id} save failed "
            logger.debug("Layer save event set: layer %d (shared=None)", layer_id)
            self.layer_save_finished_events[layer_id].set()
            self.request_queue.task_done()
            return
        group_id = task.group_id
        builder = self.group_builders[group_id] if group_id < len(self.group_builders) else self.layer_batch_builder
        ...
        self.sync_save_events[layer_id].synchronize()
        logger.info(
            "SAVE batch_copy: group=%d layer_id=%d count=%d "
            "gvas[:3]=%s addrs[:3]=%s sizes[:3]=%s",
            group_id, layer_id, len(gvas_array),
            gvas_array[:3].tolist() if hasattr(gvas_array, 'tolist') else list(gvas_array[:3]),
            addr_array[:3].tolist() if hasattr(addr_array, 'tolist') else list(addr_array[:3]),
            size_array[:3].tolist() if hasattr(size_array, 'tolist') else list(size_array[:3]),
        )
        ...
        if res != 0:
            logger.error("SAVE batch_copy FAILED: group=%d layer_id=%d ret=%d", group_id, layer_id, res)
        else:
            logger.info("SAVE batch_copy OK: group=%d layer_id=%d", group_id, layer_id)
        for req_id in req_meta.req_ids:
...
class KVCacheStoreLayerRecvingThread(KVTransferThread):
    ...
    def _handle_request(  # type: ignore[override]
        ...
        if shared is not None:
            req_meta: LayerBatchReqMeta | None = builder.build_addrs(shared, task.layer_id)
        else:
            req_meta = builder.build(task)
        if req_meta is None:
            assert not self.layer_load_finished_events[layer_id].is_set()
            logger.debug("Layer load event set: layer %d (req_meta=None)", layer_id)
            self.layer_load_finished_events[layer_id].set()
            self.request_queue.task_done()
            return
        layer_id = req_meta.layer_id
        ...
        logger.info(
            "LOAD batch_copy: group=%d layer_id=%d count=%d "
            "gvas[:3]=%s addrs[:3]=%s sizes[:3]=%s",
            group_id, layer_id, len(gvas_array),
            gvas_array[:3].tolist() if hasattr(gvas_array, 'tolist') else list(gvas_array[:3]),
            addr_array[:3].tolist() if hasattr(addr_array, 'tolist') else list(addr_array[:3]),
        ...
        if res != 0:
            logger.error("LOAD batch_copy FAILED: group=%d layer_id=%d ret=%d", group_id, layer_id, res)
        else:
            logger.info("LOAD batch_copy OK: group=%d layer_id=%d", group_id, layer_id)
        if layer_id == self.final_layer_id:
