...
class KVPoolScheduler:
    def __init__(
        ...
        self.use_layerwise_multi_group = self.use_layerwise and len(self.kv_cache_group_ids) > 1
        ...
        self.group_page_sizes: list[int] = [page_size_bytes]
        self.group_num_layers_list: list[int] = []
        ...
        if kv_cache_config is not None and self.use_hybrid:
            ...
            self.group_num_layers_list = []
            for gid, g in enumerate(kv_cache_config.kv_cache_groups):
                spec = g.kv_cache_spec
                if isinstance(spec, type(None)):
                    self.group_page_sizes.append(page_size_bytes)
                    self.group_num_layers_list.append(self.num_layers)
                    continue
                gps = getattr(spec, 'page_size_bytes', page_size_bytes)
                self.group_page_sizes.append(gps)
                self.group_num_layers_list.append(len(g.layer_names))
        else:
            self.group_num_layers_list = [self.num_layers]
    ...
    def _ensure_tracker_gvas_cover_blocks(
        ...
        Allocate GVAs per-group at group block granularity.
        Each group uses its own block_size for rehashing and its own
        page_size/num_layers for alloc_size.
        """
        ...
        if not block_hashes:
            ...
            request_tracker.block_gvas_by_group = []
            request_tracker.gva_block_offset = 0
            return

        logger.info("_ensure_tracker_gvas_cover_blocks: req=%s block_hashes=%d num_groups=%d",
                     request_tracker.req_id, len(block_hashes), len(self.grouped_block_size))
        request_tracker.block_gvas_by_group = []
        ...
        for group_id in range(num_groups):
            ...
            block_keys = [f"{self.model_name}@{group_id}@{bh.hex()}" for bh in group_block_hashes]

            if not block_keys:
                request_tracker.block_gvas_by_group.append([])
                continue
            ...
            if missing_keys:
                group_page_size = self.group_page_sizes[group_id] if group_id < len(self.group_page_sizes) else self.page_size_bytes
                group_num_layers = self.group_num_layers_list[group_id] if group_id < len(self.group_num_layers_list) else self.num_layers
                group_keys_per_block = (self.tp_size // self.put_step) * group_num_layers
                alloc_size = group_page_size * group_keys_per_block
                new_gvas = self.store_scheduler.batch_alloc(missing_keys, [alloc_size] * len(missing_keys))
                if any(gva <= 0 for gva in new_gvas):
                    raise ValueError(f"Request {request_tracker.req_id}: batch_alloc failed for group {group_id}, gvas={new_gvas}")
                for index, gva in zip(missing_indices, new_gvas):
                    block_gvas[index] = gva

            request_tracker.block_gvas_by_group.append(block_gvas)

        request_tracker.block_keys = [f"{self.model_name}@0@{bh.hex()}" for bh in get_block_hashes(block_hashes, self._get_effective_group_block_size(0), self.hash_block_size)]
        request_tracker.block_gvas = request_tracker.block_gvas_by_group[0] if request_tracker.block_gvas_by_group else []
        request_tracker.gva_block_offset = 0
    ...
    def _get_store_lookup_hit_tokens(
        ...
        num_blocks = token_len // self.hash_block_size
        ...
        query_start_block = 0 if self.use_layerwise else min(num_computed_tokens // self.hash_block_size, num_blocks)
        ...
        return num_hit_blocks * self.hash_block_size

    def _get_gate_group_ids_for_layerwise(self) -> list[int]:
        """Filter to gate groups, same logic as pool_worker._is_lookup_gate_group."""
        gate_group_ids = []
        for group_id in self.kv_cache_group_ids:
            ...
            if cache_family != "c1":
                continue
            if self._get_group_block_size(group_id) != self._block_size:
                continue
            gate_group_ids.append(group_id)
        return gate_group_ids if gate_group_ids else list(self.kv_cache_group_ids)
    ...
    def _get_layerwise_gva_hit_tokens(
        ...
        gate_group_ids = self._get_gate_group_ids_for_layerwise()
        hits = []
        for group_id in gate_group_ids:
            effective_block_size = self._get_effective_group_block_size(group_id)
            group_block_hashes = get_block_hashes(block_hashes_to_check, effective_block_size, self.hash_block_size)
            keys_to_query = [f"{self.model_name}@{group_id}@{bh.hex()}" for bh in group_block_hashes]
            ...
            for key_info in key_infos:
                sizes = key_info.size()
                exists.append(1 if (sizes and sizes > 0) else 0)

            hit_end = len(exists) * effective_block_size
            for index, value in enumerate(exists):
                if value != 1:
                    hit_end = 0
                    for hit_index in range(index, 0, -1):
                        start_pos = hit_index * effective_block_size
                        if start_pos % self.cache_transfer_granularity == 0:
                            hit_end = start_pos
                            break
                    break
            hits.append(hit_end)

        result = min(hits) if hits else 0
        logger.info(
            "_get_layerwise_gva_hit_tokens: req=%s token_len=%d "
            "gate_groups=%s hits_per_group=%s return=%d",
            request.request_id, token_len, gate_group_ids, hits, result,
        )
        return result
    ...
    def _get_effective_group_block_size(self, group_id: int) -> int:
        """Get the effective block size for a group, including cache_family ratio."""
    ...
    def get_num_new_matched_tokens(
        ...
        """
        logger.info(
            "[Layerwise get_num_new_matched_tokens] req=%s use_gva_layerwise=%s use_layerwise=%s "
            "backend_name=%s discard_partial=%s kv_role=%s computed_tokens=%d",
            request.request_id, self.use_gva_layerwise, self.use_layerwise,
            self.backend_name, self._discard_partial_chunks, self.kv_role, num_computed_tokens,
        )
    ...
    def _process_new_request(
        ...
        has_last_block = num_tokens_to_compute % self.hash_block_size != 0
        logger.info(
            "[Layerwise info] _process_new_request: req_id=%s num_tokens_to_compute=%d "
            "num_blocks=%d has_last_block=%s block_hashes=%d block_hashes_sample=%s",
            request.req_id, num_tokens_to_compute, num_blocks, has_last_block,
            len(request_real.block_hashes) if hasattr(request_real, 'block_hashes') and request_real.block_hashes else 0,
            request_real.block_hashes[:3] if hasattr(request_real, 'block_hashes') and request_real.block_hashes else None,
        )
    ...
    def _process_running_cached_request(
        ...
        has_last_block = request_tracker.token_len % self.hash_block_size != 0 or current_hash_count > len(
            request.block_hashes
        )
    ...
    def _process_async_load_request(
        ...
        if (num_tokens_to_compute % self.hash_block_size != 0) and (
        ...
        # DEBUG: Log block_hashes and allocation details for async load
        logger.debug(
            f"[Layerwise Debug] _process_async_load_request - req_id={request_id}, "
            f"num_tokens={num_tokens_to_compute}, num_blocks={num_blocks}, "
            f"has_last_block={has_last_block}"
        )
        if hasattr(request, 'block_hashes') and request.block_hashes:
            logger.debug(
                f"[Layerwise Debug] async_load - block_hashes count={len(request.block_hashes)}, "
                f"first_3={request.block_hashes[:3]}"
            )
        else:
            logger.debug("[Layerwise Debug] async_load - no block_hashes available")
