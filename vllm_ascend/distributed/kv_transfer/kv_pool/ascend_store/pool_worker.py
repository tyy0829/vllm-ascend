...
class KVPoolWorker:
    ...
    def _init_kv_transfer_config(self, vllm_config, extra_config, use_layerwise, kv_cache_config) -> None:
        ...
        self.use_layerwise_multi_group = self.use_layerwise and self.num_kv_cache_groups > 1
    ...
    def _init_layerwise_config(self) -> None:
        self.slot_group_ids: list[int] = []
        self.slot_layer_idx_in_group: list[int] = []
        self.slot_offset_by_group: dict[int, int] = {}
        self.physical_layer_to_slots: dict[int, list[int]] = {}
        self.layer_name_to_physical_layer: dict[str, int] = {}
        self.group_layer_physical_ids: dict[int, list[int]] = {}
        self.group_layer_names_stored: dict[int, list[str]] = {}

        if self.use_layerwise_multi_group and self.kv_cache_config is not None:
            _slot_offset = 0
            for group_id, group_spec in enumerate(self.kv_cache_config.kv_cache_groups):
                self.slot_offset_by_group[group_id] = _slot_offset
                self.group_layer_names_stored[group_id] = list(group_spec.layer_names)
                self.group_layer_physical_ids[group_id] = []
                for layer_idx_in_group, layer_name in enumerate(group_spec.layer_names):
                    self.slot_group_ids.append(group_id)
                    self.slot_layer_idx_in_group.append(layer_idx_in_group)
                    physical_layer = self._extract_physical_layer_index(layer_name)
                    self.layer_name_to_physical_layer[layer_name] = physical_layer
                    self.group_layer_physical_ids[group_id].append(physical_layer)
                    self.physical_layer_to_slots.setdefault(physical_layer, []).append(_slot_offset + layer_idx_in_group)
                _slot_offset += len(group_spec.layer_names)
            self.total_layerwise_slots = _slot_offset
        else:
            self.total_layerwise_slots = self.num_layers
            self.slot_offset_by_group[0] = 0
            self.slot_group_ids = [0] * self.num_layers
            self.slot_layer_idx_in_group = list(range(self.num_layers))
            for i in range(self.num_layers):
                self.physical_layer_to_slots[i] = [i]

        self.layer_load_tasks: list[list[LayerTransferTask]] = [[] for _ in range(self.total_layerwise_slots)]
        self.layer_save_tasks: list[list[LayerTransferTask]] = [[] for _ in range(self.total_layerwise_slots)]
    ...
    @staticmethod
    def _extract_physical_layer_index(layer_name: str) -> int:
    ...
    def _build_group_layer_builders(self) -> list[LayerBatchBuilder]:
        ...
        for group_id in range(num_groups):
            ...
            group_block_len = self.group_block_len.get(group_id, self.block_len)
            ...
            builder.slot_offset = self.slot_offset_by_group.get(group_id, 0)
    ...
    def _start_kv_transfer_threads(self) -> None:
        ...
        if self.use_layerwise:
            total_slots = self.total_layerwise_slots
            self.get_event = threading.Event()
            self.layer_load_finished_events = [threading.Event() for _ in range(total_slots)]
            self.layer_save_finished_events = [threading.Event() for _ in range(total_slots)]
            self.sync_save_events = [torch.npu.Event() for _ in range(total_slots)]
            self._group_builders = self._build_group_layer_builders()
            if self.use_gva_layerwise and self.kv_role in ["kv_producer", "kv_both"]:
                ...
                self.kv_send_thread = KVCacheStoreLayerSendingThread(
                    ...
                    group_builders=self._group_builders,
                )
    ...
    def _get_effective_group_block_size(self, group_id: int) -> int:
        """Get effective block size including cache_family ratio."""
    ...
    def _process_save_for_layer_batch(
        ...
        group_block_size = self._get_effective_group_block_size(group_id)
        request_block_ranges = []
        for request in requests:
            if request.can_save is None or not request.can_save:
                continue
            # Skip requests whose current group has no blocks allocated (multi-group case)
            group_block_ids = (
            ...
            save_end_block = min(request.save_end_token // group_block_size, len(group_block_ids))
            ...
            # Validate partial_block_index against this group's block_ids length
            if partial_block_index is not None and partial_block_index >= len(group_block_ids):
        ...
        if request_block_ranges:
            self.layer_save_tasks[slot_idx].append(
                LayerTransferTask(
                    layer_id=slot_idx,
    ...
    def _process_load_for_layer_batch(
        ...
        group_block_size = self._get_effective_group_block_size(group_id)
        block_hashes = requests[0].block_hashes if requests else []
        request_block_ranges = []
        for request in requests:
            ...
            load_start_block = request.load_spec.vllm_cached_tokens // group_block_size
            ...
            needs_last_block_at_boundary = (
                cached_tokens > 0 and cached_tokens % group_block_size == 0 and full_blocks < cached_full_blocks
            )
            if request.last_block_gva is not None and (
                cached_tokens % group_block_size != 0 or needs_last_block_at_boundary
            ):
                partial_block_index = (
                    cached_full_blocks if cached_tokens % group_block_size != 0 else cached_full_blocks - 1
                )
            else:
                partial_block_index = None
        ...
        if request_block_ranges:
            self.layer_load_tasks[slot_idx].append(
                LayerTransferTask(
                    layer_id=slot_idx,
                    block_ranges=request_block_ranges,
                    group_id=group_id,
    ...
    def _build_shared_save_data(self) -> None:
        """Build shared block data and attach to layer save tasks.

        For multi-group: build per-group SharedBlockData since block_ids differ
        across groups. For single-group: build once and share across all layers.
        ...
        if isinstance(self.kv_send_thread, KVCacheStoreLayerSendingThread):
            seen_groups: dict[int, SharedBlockData | None] = {}
            for slot_idx in range(self.total_layerwise_slots):
                if not self.layer_save_tasks[slot_idx]:
                    continue
                first_task = self.layer_save_tasks[slot_idx][0]
                group_id = first_task.group_id
                if group_id not in seen_groups:
                    seen_groups[group_id] = self.kv_send_thread.build_shared_data(first_task)
                shared = seen_groups[group_id]
                if shared is not None:
                    for task in self.layer_save_tasks[slot_idx]:
                        task.shared_block_data = shared
        elif isinstance(self.kv_send_thread, KVCacheStoreKeyLayerSendingThread):
            first_task = None
            for slot_idx in range(self.total_layerwise_slots):
                if self.layer_save_tasks[slot_idx]:
                    first_task = self.layer_save_tasks[slot_idx][0]
                    break
    ...
    def _build_shared_load_data(self) -> None:
        ...
        For multi-group: build per-group SharedBlockData since block_ids differ.
        For single-group: build once and share across all layers.
        """
        if not isinstance(self.kv_recv_thread, KVCacheStoreLayerRecvingThread):
            return
        seen_groups: dict[int, SharedBlockData | None] = {}
        for slot_idx in range(self.total_layerwise_slots):
            if not self.layer_load_tasks[slot_idx]:
                continue
            first_task = self.layer_load_tasks[slot_idx][0]
            group_id = first_task.group_id
            if group_id not in seen_groups:
                seen_groups[group_id] = self.kv_recv_thread.build_shared_data(first_task)
            shared = seen_groups[group_id]
            if shared is not None:
                for task in self.layer_load_tasks[slot_idx]:
                    task.shared_block_data = shared
    ...
    def _submit_ready_layer_loads(self) -> None:
        ...
        def submit_slot_load(slot_idx: int) -> None:
            if not self.layer_load_tasks[slot_idx]:
                self.layer_load_finished_events[slot_idx].set()
                return
            wait_for_save_layer = None
            attention_start_gate = None
            has_current = bool(self.physical_layer_to_slots.get(self.current_layer, []))
            if slot_idx not in self.physical_layer_to_slots.get(self.current_layer, []):
            ...
            recv_thread.add_request(
                LayerLoadTask(  # type: ignore[arg-type]
                    ...
                    transfer_tasks=self.layer_load_tasks[slot_idx],
                    layer_id=slot_idx,
        ...
        while submitted_layers < submit_count and self.next_layer_to_submit < self.num_layers:
            phys_layer = self.next_layer_to_submit
            self.next_layer_to_submit += 1
            slots = self.physical_layer_to_slots.get(phys_layer, [])
            for slot_idx in slots:
                submit_slot_load(slot_idx)
            submitted_layers += 1
    ...
    def wait_for_layer_load(self, layer_name: str | None = None) -> None:
        ...
        if layer_name is not None and self.use_layerwise_multi_group:
            phys_layer = self.layer_name_to_physical_layer.get(layer_name, self.current_layer)
        else:
            phys_layer = self.current_layer

        slots_to_wait = self.physical_layer_to_slots.get(phys_layer, [])
        for slot_idx in slots_to_wait:
            if self.layer_load_tasks[slot_idx]:
                is_finish = self.layer_load_finished_events[slot_idx].wait(timeout=10)
                if not is_finish:
                    logger.info("Layerwise slot %d load wait timed out", slot_idx)
            self.layer_load_finished_events[slot_idx].clear()
        logger.debug(">>>>>>>>>>>>>>>>>>>> clear load slots for physical layer %d", phys_layer)
    ...
    def save_kv_layer(self, connector_metadata: AscendConnectorMetadata, layer_name: str | None = None) -> None:
        ...
        slots_to_save = self.physical_layer_to_slots.get(phys_layer, [])
        for slot_idx in slots_to_save:
            self.sync_save_events[slot_idx].record()
            if self.layer_save_tasks[slot_idx]:
                for block_range in self.layer_save_tasks[slot_idx][0].block_ranges:
                    send_thread.add_stored_request(block_range.request.req_id)
                send_thread.add_request(self.layer_save_tasks[slot_idx])  # type: ignore[arg-type]
            else:
                self.layer_save_finished_events[slot_idx].set()

        if self.current_layer == self.num_layers - 1:
            final_slots = self.physical_layer_to_slots.get(self.num_layers - 1, [self.num_layers - 1])
            last_slot = max(final_slots) if final_slots else self.total_layerwise_slots - 1
            is_finish = self.layer_save_finished_events[last_slot].wait(timeout=10)
            if not is_finish:
                logger.info("Layerwise save wait timed out at physical layer %d", self.current_layer)
            for slot_idx in range(self.total_layerwise_slots):
                if self.layer_save_finished_events[slot_idx].is_set():
                    logger.debug(">>>>>>>>>>>>>>>>>>>> clear save slot %d", slot_idx)
                    self.layer_save_finished_events[slot_idx].clear()
