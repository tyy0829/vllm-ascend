            ...
            for group_id in range(self.num_kv_cache_groups):
                ...
                for idx, (ki, key) in enumerate(zip(key_infos, keys)):
                    ...
                    if gva > 0:
                        ...
                        logger.warning(
                            "load_gvas: req=%s group=%d got invalid gva=%d (size=%d), "
                            "block_id=%s load failed",
                            request.req_id, group_id, gva,
                ...
                if valid_keys_for_lease:
                    lease_results = self.m_store.batch_add_lease(
                        valid_keys_for_lease, LAYERWISE_READ_LEASE_TTL_MS
                    )
                    ...
                    for i, lease_res in enumerate(lease_results):
                        if lease_res != 0 and i < len(valid_block_ids):
                            ...
                            logger.warning(
                                "load_gvas: req=%s group=%d lease failed result=%d, "
                                "block_id=%d load failed",
                                request.req_id, group_id, lease_res, valid_block_ids[i],
                            )
                else:
                    lease_results = []
